...
def readme_block(results: Results) -> str:
    return "\n\n".join(
        [
            ...
            DEPENDENCY_NOTE,
            CAVEATS,
            "RSS includes the interpreter, imports, decoded corpus and native allocations. "
            "Import time excludes interpreter startup; filesystem caches may be warm. "
            "Wheel size is the package alone; installed size includes transitive dependencies, "
            "excluding interpreter, installer tools and generated bytecode.",
            "See [methodology and reproduction](tools/benchmark/README.md), "
            "[detailed results and failures](tools/benchmark/results/macos-arm64.md), and "
            "[raw measurements](tools/benchmark/results/macos-arm64.json).",
            END,
        ]
    )
